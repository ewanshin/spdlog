...
/*
 * Generator of hourly log file names in format basename.YYYY-MM-DD-HH-MM.ext
 */
...
public:
    ...
    hourly_file_sink(filename_t base_filename, int rotation_hour, int rotation_minute, bool truncate = false)
...
{
    return Factory::template create<sinks::hourly_file_sink_mt>(logger_name, filename, 0, 0, truncate);
}
...
inline std::shared_ptr<logger> hourly_logger_st(
    const std::string &logger_name, const filename_t &filename, bool truncate = false)
{
    return Factory::template create<sinks::hourly_file_sink_st>(logger_name, filename, 0, 0, truncate);
}
} // namespace spdlog
